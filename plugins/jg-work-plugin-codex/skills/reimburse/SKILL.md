...
3. 对每张票做判断（脚本做不了的，Claude 来定）：
   ...
   - **出差天数与实际工作天数**：OA 的两个字段必须分别填写，且只能填 0.5 的倍数。
     - `出差天数`与差旅补贴天数使用同一口径，按制度 4.3 的发车时间和 12 点规则计算。制度没有规定周末停止发放差旅补贴，不能擅自从出差天数或补贴中扣除周末。
     - `实际工作天数`以出差天数为起点，扣除提前到达后未工作、周末休息、法定节假日休假等时间。节假日实际工作可以计入。
     - 行程跨周末、法定节假日，或材料显示提前到达时，必须询问用户哪些日期实际工作，确认后再生成终稿。不能只看日历自动认定是否工作。
     - 调用 `default_header("差旅", ...)` 时必须显式传入 `travel_days=<出差天数>` 和 `actual_work_days=<实际工作天数>`。`actual_work_days` 不得大于 `travel_days`。
     ...
     **费用分类规范名（照真实报销单）**：交通 `差旅费-交通费`、住宿 `差旅-住宿费`、补贴 `补贴`（差旅内；非差旅市内交通是 `市内交通费`）。
     ...
     write_oa_travel_detail(trips, items, "<输出目录>/09-终稿-OA差旅报销明细.xlsx",
                            header_fields=default_header(
                                ...
                                reason="..",
                                applicant="<本次实际报销人>",
                                travel_days=<按制度 4.3 计算的出差天数>,
                                actual_work_days=<扣除提前到达和休假后的实际工作天数>,
                            ))
     ```
...
- 出差天数与补贴天数使用制度 4.3 口径；实际工作天数单独计算，扣除提前到达后未工作、周末和法定节假日休假，只能填 0.5 的倍数。跨周末、节假日或存在提前到达时先问用户实际工作安排。
