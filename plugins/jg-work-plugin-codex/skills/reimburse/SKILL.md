...
每次开始一笔**新的报销**时，在读取票据、询问报销信息或生成文件前，先运行一次：

```bash
python3 "<本 Skill 目录>/scripts/check_update.py"
```

只在新报销开场检查一次。同一笔报销从摘要确认进入终稿阶段时不重复检查，也不循环重试。根据脚本返回的 JSON 处理：

- `current` / `checked`：继续处理本次报销，不需要单独打扰用户。
- `updated`：停止本次处理，明确告诉用户“报销 Skill 已更新，请新建任务后重新发起本次报销”。当前任务不得继续套用更新后的规则，因为本任务已载入旧版 Skill。
- `check_failed`：明确告诉用户更新检查失败及简短原因，继续使用当前版本处理，不阻塞报销。
- `not_configured`：告诉用户尚未配置 `jg-work` 插件市场，可按操作手册完成一次安装；继续使用当前版本处理，不阻塞报销。

自动更新只刷新已配置的 `jg-work` 插件市场，不修改用户的报销文件、Skill 源码仓库或其他本地目录。若运行环境要求确认网络访问或软件更新权限，按系统提示申请，不得绕过。
...
报销、财务和 OA 表单必须保留法定公司名称。workspace 中“正式材料用「公司」代称”的通用规则不适用于这类表单，也不得在 writer 生成后用通用写作规则替换标题。

- 通用 OA 明细：`上海巨耕-报销金额明细`
- 差旅 OA 明细：`上海巨耕-差旅报销明细`
- 交通费明细：`上海巨耕-交通费报销明细`

所有修改完成后，必须把标题校验作为交付前最后一个只读检查。校验失败就停止交付：

```python
from src.final_checks import validate_final_workbook_titles

validate_final_workbook_titles([
    ("<OA 明细路径>", "oa_travel"),       # 通用报销改用 oa_general
    ("<交通费明细路径>", "travel_detail"),
])
```
...
7. 生成产出物：
   ...
   - **A4 拼贴按打印份数拆分并按类型相邻排列**：直接从原始 `tickets`（自带 `is_prepaid`）过滤，跳过预充值票、住宿水单/行程单/订阅回执等非发票附件，也跳过没有 PDF 的票。普票进入打印 1 张文件；专票和铁路电子客票进入整份打印 2 次文件。铁路电子客票仍保留原票种，不参与专票税额计算。高铁票需准备两份纸质件，一份粘贴，一份随报销单交财务。各文件内按票据类型稳定分组：高铁票连续放在一起，飞机票连续放在一起，打车票连续放在一起，住宿票连续放在一起，其余票据放最后。同类型内部保持摘要顺序：
     ```python
     from src.build_a4 import build_a4, split_print_specs
     ...
     normal_specs, two_copy_specs = split_print_specs(valid)
     if normal_specs:
         build_a4(normal_specs, "<输出目录>/09-终稿-A4发票-普票-打印1张.pdf")
     if two_copy_specs:
         build_a4(two_copy_specs, "<输出目录>/09-终稿-A4发票-专票及高铁票-整份打印2次.pdf")
     ```
     `a4_group` 在组装票据时按内容确定：铁路电子客票=`高铁`，航空运输票据对应发票=`飞机`，客运服务打车票=`打车`，住宿服务发票=`住宿`，其余=`其他`。两个 PDF 内每张发票只放一次，不在文件中复制；打印时把“专票及高铁票”整份 PDF 打印 2 次。`invoice_kind="unknown"` 时先读取 PDF 标题确认：标题含“增值税专用发票”归专票，含“普通发票”归普票；仍无法确认时让用户判断，不得混入任一打印文件。某类没有发票时不生成空 PDF。新文件检查通过后删除旧的混合 `A4拼贴.pdf`/`A4发票拼贴.pdf`，避免重复打印。任何因没有 PDF 被跳过的发票，要明确提醒用户。
...
- 法定标题：报销表单保留“上海巨耕”，不得替换成“公司”；交付前最后一步调用 `validate_final_workbook_titles`，检查通用 OA、差旅 OA、交通费明细的 `A1` 精确标题。
- 份数与文件：普票进入 `09-终稿-A4发票-普票-打印1张.pdf`；专票和铁路电子客票进入 `09-终稿-A4发票-专票及高铁票-整份打印2次.pdf`。两个 PDF 内每张发票都只放 1 次；后一个文件由用户整份打印 2 次。高铁票一份粘贴，一份随报销单交财务。铁路电子客票不改票种、不计专票税额。某类为空则不生成。
...
- A4 拼贴先分打印 1 张/整份打印 2 次，再按高铁、飞机、打车、住宿、其他分组排列；同类型票据连续放置。
