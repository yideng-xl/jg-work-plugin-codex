...
3. 对每张票做判断（脚本做不了的，Claude 来定）：
   ...
   - **起止地**：先按交通类型归一化，再填 `from_`/`to`。
     ...
     - 市内打车保留行程单的 POI 原文。行程单写商场、公交站、酒店、茶馆等，就按原文判断和填写，不主动简化成地点类型。
     - 打车定位存在偏差。结合行程单的上下车点和其他票据判断附近地点；只有证据能明确证明 POI 与实际地点不一致时才改，并在摘要阶段让用户确认。
     - 判断不清时保留 POI 原文，设置 `loc_uncertain=True` 标黄让用户确认，不猜。
   ...
   - **高铁/飞机方向与时刻**：票面文本解析结果只作为候选值。用票面日期 + 车次/航班号查询公开时刻表，确认出发地、目的地和计划到达时间；飞机按计划到达时间填写，不追踪实际延误。查询优先使用铁路、航空公司、机场等公开页面；找不到时再用可信的公开出行平台，并在交付说明中列为待用户核对。摘要和终稿只写确认后的方向、发车/起飞时间、到达时间，不写`按票面纠正解析方向`、`解析结果`、`估算`等过程说明。高铁备注只保留必要业务信息，例如`G0001，08:00 发车`。
...
- 起止地：跨城交通填城市；市内打车保留行程单 POI 原文，证据明确时才纠正。
